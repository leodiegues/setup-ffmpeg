...
if [ $RUNNER_OS = macOS ]; then
  if [ $RUNNER_ARCH = ARM64 ]; then
    ...
    if [ $version = master ]; then
      ...
      curl -H "User-Agent: Mozilla/5.0" -v -L -o FFmpeg.zip https://www.osxexperts.net/ffmpeg${version}arm.zip
      sleep 5
      curl -H "User-Agent: Mozilla/5.0" -v -L -o FFprobe.zip https://www.osxexperts.net/ffprobe${version}arm.zip
    fi
  else
    ext=7z
    if [ $version = master ]; then
