...
if [ $RUNNER_OS = macOS ]; then
  if [ $RUNNER_ARCH = ARM64 ]; then
	ext=zip
	echo ::debug::Setting extension to "zip"
	if [ $version = master ]; then
	  echo ::error::OSXExperts.NET currently does not have macOS ARM64 master builds.
	  exit 1
	else
	  echo ::debug::Downloading FFmpeg from OSXExperts.NET
	  echo ::debug::Download URLs:
	  echo ::debug::https://www.osxexperts.net/ffmpeg${version}arm.zip
	  echo ::debug::https://www.osxexperts.net/ffprobe${version}arm.zip
	...
	ext=7z
	echo ::debug::Setting extension to "7z"
	echo ::debug::Downloading FFmpeg from Evermeet.cx
	if [ $version = master ]; then
		echo ::debug::Download URLs:
		echo ::debug::https://evermeet.cx/ffmpeg/get
		echo ::debug::https://evermeet.cx/ffmpeg/get/ffprobe
	  wget -qO FFmpeg.7z https://evermeet.cx/ffmpeg/get
	  wget -qO FFprobe.7z https://evermeet.cx/ffmpeg/get/ffprobe
	else
		echo ::debug::Download URLs:
		echo ::debug::https://evermeet.cx/ffmpeg/ffmpeg-$version.7z
		echo ::debug::https://evermeet.cx/ffprobe/ffprobe-$version.7z
  ...
  echo ::debug::Extracting FFmpeg and FFprobe
  7z e FFmpeg.$ext ffmpeg -oFFmpeg
  7z e FFprobe.$ext ffprobe -oFFmpeg
  echo ::debug::Removing FFmpeg and FFprobe original files
